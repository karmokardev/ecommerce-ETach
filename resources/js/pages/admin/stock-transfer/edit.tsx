...
interface EditStockTransferProps {
    transfer: StockTransfer;
    warehouses: Warehouse[];
}

export default function EditStockTransfer({ transfer, warehouses }: EditStockTransferProps) {
    ...
    const addItem = () => {
        setData('items', [...data.items, { product_variant_id: 0, quantity: 1 }]);
    };
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        put(`/stock-transfers/${transfer.id}`, {
            onSuccess: () => {
            ...
            <div className="p-6">
                <div className="max-w-6xl mx-auto">
                    <div className="flex justify-between items-center mb-6">
                        <h1 className="text-2xl font-bold dark:text-white">Edit Stock Transfer</h1>
                    </div>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                ...
                                <div>
                                    ...
                                    <select
                                        ...
                                        onChange={(e) => setData('from_warehouse_id', parseInt(e.target.value))}
                                        className="w-full px-3 py-2 border border-gray-300 dark:border-neutral-700 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent bg-white dark:bg-neutral-800 text-gray-900 dark:text-gray-100"
                                    >
                                ...
                                <div>
                                    ...
                                    <select
                                        ...
                                        onChange={(e) => setData('to_warehouse_id', parseInt(e.target.value))}
                                        className="w-full px-3 py-2 border border-gray-300 dark:border-neutral-700 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent bg-white dark:bg-neutral-800 text-gray-900 dark:text-gray-100"
                                    >
                                            ...
                                            <div key={index} className="grid grid-cols-1 md:grid-cols-3 gap-4 p-4 bg-gray-50 dark:bg-neutral-800 rounded-lg">
                                                <div>
                                                    ...
                                                    <select
                                                        ...
                                                        onChange={(e) => updateItem(index, 'product_variant_id', parseInt(e.target.value) || 0)}
                                                        className="w-full px-3 py-2 border border-gray-300 dark:border-neutral-700 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent bg-white dark:bg-neutral-800 text-gray-900 dark:text-gray-100"
                                                    >
                                                        <option value="">Select Variant</option>
                                                        {transfer.items?.map((existingItem) => (
                                                            <option key={existingItem.product_variant_id} value={existingItem.product_variant_id}>
                                                                {existingItem.product_variant?.sku} - {existingItem.product_variant?.product?.name}
                                                            </option>
                                                        ))}
                                                    </select>
                                                </div>
                            ...
                            {transfer.status === 'completed' && (
                                <div className="mt-6 p-4 bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-800 rounded-lg">
                                    <p className="text-sm text-yellow-800 dark:text-yellow-200">
                                        <strong>Warning:</strong> This transfer is already completed. Editing will only update the record. 
                                        Stock will not be re-processed.
                                    </p>
                                </div>
                            )}
